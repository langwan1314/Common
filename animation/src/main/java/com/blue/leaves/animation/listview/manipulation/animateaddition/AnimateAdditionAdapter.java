...
package com.blue.leaves.animation.listview.manipulation.animateaddition;
...
import com.blue.leaves.nineoldandroids.animation.AnimatorListenerAdapter;
...
import com.blue.leaves.nineoldandroids.animation.ValueAnimator;
...
/**
 * An adapter for inserting rows into the {@link android.widget.ListView} with an animation. The root {@link android.widget.BaseAdapter}
 * should implement {@link Insertable},
 * otherwise an {@link IllegalArgumentException} is thrown. This class only works with an instance of {@code ListView}!
 * 插入的时候带一个动画，根adapter必须实现Insertable接口，否则会抛出异常，只有和ListView一起才能工作
 * <p/>
 * Usage:<br>
 ...
 * 使用步骤：
 * a、使用一个BaseAdapter来初始化一个该动画实例
 * b、使用setListView为该实例设置一个ListView
 * c、调用insert方法执行插入操作
 * <p/>
...
public class AnimateAdditionAdapter<T> extends BaseAdapterDecorator {
     ...
     * 设置list是否应该往下开启动画，当添加的item在第1个可见item的上面
     *
     ...
     * Will also call {@link Insertable#add(int, Object)} of the root {@link android.widget.BaseAdapter}.
     *
     * @param indexItemPairs the index-item pairs to insert. The first argument of the {@code Pair} is the index, the second argument is
     *                       the item.
     */
    ...
    public void insert(@NonNull final Iterable<Pair<Integer, T>> indexItemPairs) {
        ...
        for (Pair<Integer, T> pair : indexItemPairs) {
            ...
            } else if (getListViewWrapper().getLastVisiblePosition() >= pair.first || getListViewWrapper().getLastVisiblePosition() ==
        ...
        if (mShouldAnimateDown) {
            ((AbsListView) getListViewWrapper().getListView()).smoothScrollBy(scrollDistance, (int) (mScrolldownAnimationDurationMs *
                    numInsertedAbove));
        }
     ...
     * 当动画结束时，从InsertQueue移除active的index
     */
