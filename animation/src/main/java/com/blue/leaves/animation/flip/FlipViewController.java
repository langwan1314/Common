...
package com.blue.leaves.animation.flip;
...
import com.blue.leaves.animation.R;
import com.blue.leaves.animation.flip.utils.AphidLog;

import junit.framework.Assert;

import java.util.LinkedList;

public class FlipViewController extends AdapterView<Adapter> {//代表一个item View 中的控制器部分，主要用于
     ...
     */
    public FlipViewController(Context context, AttributeSet attrs, int defStyle) {//构造函数
    ...
    // Touch Event
    @Override
    public boolean onInterceptTouchEvent(MotionEvent event) {//触摸事件交给card
    ...
    public void setSelection(int position) {
        ...
        for (int i = 1; i <= sideBufferSize; i++) {//添加前后页到buffer（sideBufferSize =1）
        ...
        cards.resetSelection(position, adapterDataCount);//设置选中页
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {//布局
        ...
        if (bufferedViews.size() >= 1) {//更新bufferView
    ...
    //--------------------------------------------------------------------------------------------------------------------
    // Internals
    private void setupSurfaceView(Context context) {//初始化
    ...
    private void addReleasedView(View view) {//添加到releasedViews,重用
        Assert.assertNotNull(view);
    ...
    private View viewFromAdapter(int position, boolean addToTop) {//从adapter获取view
        ...
        setupAdapterView(view, addToTop, view == releasedView);//addViewInLayout后还要对子Viewlayout一下 规定它的上下左右的位置 这样才能正确显示到屏幕上
        return view;
    }

    private void setupAdapterView(View view, boolean addToTop, boolean isReusedView) {//用于添加view或者重新绑定view
        ...
        if (isReusedView) {
            attachViewToParent(view, addToTop ? 0 : 1, params);//已经添加过，重新attach
        } else {
            addViewInLayout(view, addToTop ? 0 : 1, params, true);//没有添加过，则添加
        }
    }

    private void updateVisibleView(int index) {//设置view的可见性
    ...
    void flippedToView(final int indexInAdapter, boolean isPost) {//翻转后处理(善后工作)
        if (AphidLog.ENABLE_DEBUG) {
        ...
        if (indexInAdapter >= 0 && indexInAdapter < adapterDataCount) {
            ...
            if (indexInAdapter == adapterIndex + 1) { //forward one page
                if (adapterIndex < adapterDataCount - 1) {
                    adapterIndex++;
                    View old = bufferedViews.get(bufferIndex);//获取原来的view
                    if (bufferIndex + 1 > sideBufferSize) {//sideBufferSize =1
                        releaseView(bufferedViews.removeFirst());//移除原来的
                    }
                    if (adapterIndex + sideBufferSize < adapterDataCount) {
                        bufferedViews.addLast(viewFromAdapter(adapterIndex + sideBufferSize, true));//添加后一页到bufferedViews
                    }
    ...
    void showFlipAnimation() {//显示滑动动画，主要是把滑动动画的view设置为visible进行绘制
        if (!inFlipAnimation) {//是否已经在显示中
            inFlipAnimation = true;

            cards.setVisible(true);//设置为可见
    ...
    private void hideFlipAnimation() {//关闭滑动动画，主要是把滑动动画的view设置为invisible进行绘制
        if (inFlipAnimation) {
            inFlipAnimation = false;

            updateVisibleView(bufferIndex);//更新可见的View
